...
const FUNC_RE =
  /(^|\n)([\s]*(?:(?:public|private|protected|static|abstract|final|readonly)\s+)*)function\s+(\w+)\s*\(/g;

const CLASS_RE = /\b(?:class|trait|interface)\s+(\w+)/g;

export function buildPhpFunctionTable(filePath: string, text: string): FunctionTable {
  ...
  while ((m = FUNC_RE.exec(text))) {
    ...
    const enclosing = innermost(classRanges, declStart, closeIdx);
    const qualified = enclosing ? `${enclosing.name}.${name}` : name;
    // PHP top-level functions are always global. In a class, public by default; explicit private wins.
