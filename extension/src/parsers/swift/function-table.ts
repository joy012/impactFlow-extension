...
const FUNC_RE =
  /(^|\n)([\s]*(?:@[\w()]+\s*)*(?:(?:public|private|internal|fileprivate|open|static|class|final|override|mutating|nonmutating|required|convenience)\s+)*)func\s+(\w+)\s*(?:<[^>]+>)?\s*\(([^)]*)\)\s*(?:async\s*)?(?:throws\s*)?(?:->\s*[^{]+)?\s*\{/g;

const TYPE_RE = /\b(?:class|struct|enum|protocol|extension|actor)\s+(\w+)/g;

export function buildSwiftFunctionTable(filePath: string, text: string): FunctionTable {
  ...
  while ((m = FUNC_RE.exec(text))) {
    ...
    const enclosing = innermost(typeRanges, declStart, closeIdx);
    const qualified = enclosing ? `${enclosing.name}.${name}` : name;
    const isExported = !/\bprivate\b|\bfileprivate\b/.test(modifiers);
